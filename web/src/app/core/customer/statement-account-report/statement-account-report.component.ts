...
@Component({
  selector: "app-statement-account-report",
  templateUrl: "./statement-account-report.component.html",
  styleUrls: ["./statement-account-report.component.scss"]
})
export class StatementAccountReportComponent implements OnInit {
  entries: number = 5;
  selected: any[] = [];
  temp = [];
  activeRow: any;
  ...
  clickCompanyName(companyname) {
    let val = companyname.toLowerCase();
    this.temp = this.rows.filter(function(d) {
      if (d.companyname.toLowerCase().indexOf(val) !== -1) {
        return true;
      }
      return false;
    });
