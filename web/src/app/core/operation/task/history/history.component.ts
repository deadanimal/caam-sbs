...
export class HistoryComponent implements OnInit {
  ...
  getFileUpload(user_obj) {
    this.fileuploadService
      ...
      .subscribe((res) => {
        ...
        this.temp = this.rows.filter(obj => {
          return obj.status == 'FIL1' || obj.status == 'FIL0';
        });
      });
  }
  ...
  onTabSelect(data: TabDirective) {
    if (data.heading == 'New Task') {
      this.temp = this.rows.filter(obj => {
        return obj.status == 'FIL1' || obj.status == 'FIL0';
      });
    }
    else if (data.heading == 'Processed Task') {
      this.temp = this.rows.filter(obj => {
        return obj.status == 'FIL2';
      });
    }
  }
  ...
  view(row) {
    ...
    this.router.navigate(["operation/task/tflvfr/view"], navigationExtras);
  }
